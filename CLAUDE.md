...
- Auth: bcrypt signup/login backed by the Mongo `users` collection — OPTIONAL by default (guest mode with a sidebar login expander); the `REQUIRE_LOGIN` secret set to "true" restores the hard gate (Atlas in the cloud; `MONGODB_URI` comes from Streamlit secrets, with certifi CA for TLS). `_mongo_client` is `@st.cache_resource` and deliberately raises on failure so a transient Atlas error isn't cached until reboot.
